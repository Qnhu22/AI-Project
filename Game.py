...
def ai_move(auto_move_path, auto_move_index, maze_matrix, boat):
    if auto_move_path is not None and auto_move_index < len(auto_move_path):
        ...
        print(f"AI moved to {boat.row}, {boat.col}")
...
def reset_game():
    global player_step_counter, AI_step, keys, collected_keys, algorithm_selected, game_over, player_won, ai_active, start_time, num_keys, show_algorithm_panel, monster_start_time, monster_elapsed_time, show_path, path_printed, is_reset_allowed
    ...
    boat.path = None
    boat.path_index = 0
    boat.monster_path = [(boat.row, boat.col)]  # Khởi tạo lại monster_path trong Boat
...
last_debug_time = pygame.time.get_ticks()  # Biến để kiểm soát thời gian debug
show_algorithm_panel = False
monster_start_time = None
monster_elapsed_time = 0
show_path = False
...
last_ai_move_time = pygame.time.get_ticks()  # Biến để kiểm soát thời gian di chuyển AI
...
while True:
        ...
        if event.type == pygame.QUIT:
            ...
            if show_algorithm_panel:
                ...
                if buttons["reset"].collidepoint(event.pos):
                    ...
                    print("Show Path button clicked")
                    if game_over:
    ...
    # Chỉ in debug mỗi giây một lần
    if current_time - last_debug_time >= 1000:  # 1000ms = 1 giây
        print(f"Elapsed time: {elapsed_time}, Algorithm selected: {algorithm_selected}, Game over: {game_over}, AI active: {ai_active}")
        last_debug_time = current_time

    # Chỉ kích hoạt AI sau 5 giây nếu chưa active
    ...
    # Kiểm soát thời gian di chuyển của AI
    if ai_active and not game_over:
        if current_time - last_ai_move_time >= 150:  # Đồng bộ với move_delay trong Boat.py
            try:
